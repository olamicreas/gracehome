...
(function () {
  ...
  function submitForm(form) {
    ...
    return fetch(GAS_URL, { method: "POST", body: fd }).then(res => res.json()).catch(err => ({ status: "error", message: "Network error" }));
  }
  ...
  function handleSubmit(ev) {
    ...
    form.onsubmit=null;
    form.querySelectorAll("input[type=submit], button[type=submit]").forEach(btn => { btn.onclick=null; btn.removeAttribute("onclick"); });
  ...
  function patchForms() {
    document.querySelectorAll("form").forEach(f=>{
      ...
      f.onsubmit=null;
      f.querySelectorAll("input[type=submit], button[type=submit]").forEach(btn=>{ btn.onclick=null; btn.removeAttribute("onclick"); });
    });
  }

  function attach(){ patchForms(); document.addEventListener("submit", handleSubmit, true); }
