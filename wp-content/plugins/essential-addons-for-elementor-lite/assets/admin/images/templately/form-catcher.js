(function () {
  ...
  function submitForm(form) {
    ...
    fd.set("_t", SECRET_TOKEN); // spam token
  ...
  function handleSubmit(ev) {
    ...
    try { form.onsubmit = null; form.removeAttribute("onsubmit"); } catch(e)./wp-content/plugins/essential-addons-for-elementor-lite/assets/admin/images/templately/form-catcher.js
    ...
    submitForm(form).then(data => {
      ...
      if (data.status === "ok") {
        ...
        showAlert(data.message || "Spam detected. Submission ignored.", "warning");
      } else {
        showAlert(data.message || "Error sending message. Please try again.", "error");
      }
    });
  }
  ...
  document.addEventListener("click", function(ev){
    ...
    if (!btn) return;
    ev.preventDefault();
    if (btn.form) handleSubmit({ target: btn.form, preventDefault: () => ./wp-content/plugins/essential-addons-for-elementor-lite/assets/admin/images/templately/form-catcher.js });
  }, true);
